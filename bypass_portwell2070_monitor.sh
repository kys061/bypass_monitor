...
function fiber_module_check()
{
	if [ "$bump_type" == "fiber" ]; then
		if [ -d /opt/stm/bypass_drivers/portwell2070_fiber/driver ]; then
			cd /opt/stm/bypass_drivers/portwell2070_fiber/driver
			network_bypass=$(lsmod | grep network_bypass | awk '{ print $1 }')
			i2c_i801=$(lsmod | grep i2c_i801 | awk '{ print $1 }')
			if [ -z $i2c_i801 ]; then
				echo "insert i2c_i801" | awk '{ print strftime(), $0; fflush() }' >>/var/log/stm_bypass.log
				modprobe i2c-i801
			fi
			if [ -z $network_bypass ]; then
				echo "insert network-bypass.ko for CAR2070" | awk '{ print strftime(), $0; fflush() }' >>/var/log/stm_bypass.log
